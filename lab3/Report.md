...
### Особенности реализации в Gleam:

- Строгая типизация предотвращает множество ошибок на этапе компиляции
- Система модулей обеспечивает чёткую структуру проекта
- FFI (Foreign Function Interface) для взаимодействия с Erlang VM позволяет эффективно работать с вводом/выводом
- Отсутствие исключений заставляет явно обрабатывать все ошибочные ситуации

Функциональный подход к потоковой обработке данных показал свою эффективность и элегантность. Программа получилась компактной, понятной и легко расширяемой.
